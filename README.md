...
- **Records-law-aware deletion:** nothing is ever silently destroyed. A "deleted"
  update is recoverable, not gone, and a permanent, admin-only audit log tracks every
  delete, restore, and purge, with who did it and when. Built for agencies where
  routine software use could otherwise become an accidental public-records problem.
...
framework, just server-rendered HTML with vanilla CSS/JS. Deliberately simple: this is
